...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const url = `https://car-land-server-production.up.railway.app/product/${productId}`;
    fetch(url)
      .then((res) => res.json())
  ...
  const handleDelivered = (e) => {
    ...
    const url = `https://car-land-server-production.up.railway.app/quantity/${productId}`;
    fetch(url, {
      method: "PUT",
      headers: {
  ...
  return (
    <div>
      ...
      <div className="d-flex px-5 my-4 mx-auto">
        <img src={product.picture} style={{ width: "250px" }} alt="" />
